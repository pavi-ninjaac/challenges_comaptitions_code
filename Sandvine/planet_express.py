...
class Employer:
    ...
    def report_package_delivered(self , package):
        self.balance -= 1
        package.worker.balance += 1 #change the place i think
        try:
            package.mark_delivered()
        except Exception as e:
            self.logger.warning(f"A problem occurred while delivering the package: {e}") #error raised need to reduce here
